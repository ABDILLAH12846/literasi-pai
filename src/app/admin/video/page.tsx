...
export default function AdminVideoPage() {
  ...
  async function handleDelete(id: number) {
    await fetch(`/api/video?id=${id}`, { method: "DELETE" });
    ...
    <div className="p-4 sm:p-8">
      <div className="mx-auto max-w-5xl">
        <div className="mb-6 flex flex-col gap-3 sm:mb-8 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 sm:text-3xl">Kelola Video Islami</h1>
            <p className="mt-1 text-sm text-gray-500">Tambah, edit, dan hapus video Islami</p>
          </div>
          <button
          ...
          <form onSubmit={handleSubmit} className="mb-6 rounded-xl border border-gray-200 bg-white p-4 shadow-sm sm:mb-8 sm:p-6">
            <div className="mb-3 flex items-center justify-between sm:mb-4">
              <h2 className="text-base font-bold text-gray-900 sm:text-lg">
                {editingId ? "Edit Video" : "Video Baru"}
              </h2>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4">
              <div className="sm:col-span-2">
                <label className="mb-1 block text-xs font-medium text-gray-700 sm:text-sm">Judul Video</label>
                <input
                  type="text"
                  value={form.title}
              ...
              <div className="sm:col-span-2">
                <label className="mb-1 block text-xs font-medium text-gray-700 sm:text-sm">URL YouTube</label>
                <input
                  type="url"
              ...
              <div>
                <label className="mb-1 block text-xs font-medium text-gray-700 sm:text-sm">URL Thumbnail (opsional)</label>
                <input
                  type="url"
            ...
            <div className="mt-3 flex justify-end gap-2 sm:mt-4">
              ...
              <button type="submit" className="rounded-lg bg-emerald-600 px-5 py-2 text-xs font-medium text-white transition-colors hover:bg-emerald-700 sm:px-6 sm:py-2.5 sm:text-sm">
                {editingId ? "Simpan Perubahan" : "Simpan Video"}
              </button>
            </div>
          </form>
        )}
            ...
            <table className="w-full min-w-[650px]">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  ...
                  <th className="px-4 py-3 text-left text-[10px] font-semibold uppercase tracking-wider text-gray-500 sm:px-6 sm:text-xs">URL</th>
                ...
                {videos.length === 0 ? (
                  ...
                  videos.map((v) => (
                    <tr key={v.id} className="hover:bg-gray-50">
                      <td className="px-4 py-3 text-xs font-medium text-gray-900 sm:px-6 sm:py-4 sm:text-sm">{v.title}</td>
                      ...
                      <td className="max-w-[150px] truncate px-4 py-3 text-xs text-gray-500 sm:max-w-none sm:px-6 sm:py-4 sm:text-sm">
                        <a href={v.youtubeUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
                          {v.youtubeUrl}
                        </a>
                      </td>
